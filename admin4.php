...
<nav class="navbar navbar-expand-lg navbar-light bg-success">
  ...
  <div class="tbl">
        ...
        <?php 
            $query = "select * from tbl_admin4";
            $result = $db ->select($query);
            if($result !=false){
                while($value = mysqli_fetch_array($result)){
                    $id = $value['id'];
                    $fourth_table = $value['time'];
        ?>
        <tr>
                  <td><?= $value['name']; ?></td>
        ...
        <?php
            ...
            if($result !=false){
                ...
                $subtraction_result =$fourth_table - $third_table;
                $addition           = $total_time + $subtraction_result;
        ?>
        <?php
            $query = "UPDATE tbl_admin4 SET total_time='$addition' Where id='$id'";
    ...
    <form  method="post" action="auth4.php">
    ...
    <div class="mt-2">
    <a href="admin3.php" class="btn btn-dark">Next Branch</a>
    </div>
  </div>  
</body>
</html>